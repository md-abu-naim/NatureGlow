    ...
    <div>
      <div className="flex justify-center items-center min-h-screen px-4">
        <div className="bg-green-50 shadow-lg border border-green-500 rounded-2xl w-full max-w-2xl text-center">
          <img src={cover} alt="Cover" className="w-full h-36 object-cover rounded-t-2xl" />
          <div className="-mt-14 flex flex-col items-center">
            <img src={profile} alt="Profile" className="h-28 w-28 rounded-full border-4 border-green-500 object-cover" />
            <p className="mt-2 px-3 py-1 bg-green-200 text-green-700 rounded-full text-sm">
              Role: <span className="font-medium">{role || "Anonymous"}</span>
            </p>
            <h2 className="mt-3 text-2xl font-semibold text-black">{name || "Anonymous"}</h2>
            <p className="text-gray-700 text-sm"><span className="font-semibold">Email: </span> {email}</p>
            <p className="text-gray-700 text-sm"><span className="font-semibold">User ID: </span> {userId}</p>

            <div className={`flex flex-col md:flex-row justify-center gap-4 ${role === 'Admin' ? 'my-6' : 'mt-6'}`}>
