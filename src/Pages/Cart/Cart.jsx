...
const Cart = () => {
  ...
  <div className='grid grid-cols-1 lg:grid-cols-3 gap-4 mt-7'>
    <div className='lg:col-span-2 space-y-4 p-5 rounded-xl bg-green-100'>
      {
        cartItems?.map(item => (
          <div key={item.id} className='flex flex-col md:flex-row text-center md:text-start items-center p-4 gap-4 shadow-md rounded-xl border border-green-600'>
            <img className='w-28 h-28 object-cover rounded-xl border border-green-500' src="https://i.postimg.cc/25BPjPQg/Aloe-Neem-Anti-Dandruff-Shampoo.webp" alt="" />
            <div className='flex-1'>
              <h2 className='text-lg font-semibold text-green-700'>{item.name}</h2>
              <p className='text-sm text-gray-700 font-medium italic'>{item.category}</p>
    ...
    {/* Order Summary */}
    <div className="w-full lg:col-span-1"> {/* Full width on mobile */}
      <div className='bg-green-100 p-4 rounded-xl w-full'>
        <div className='p-6 shadow-md rounded-xl border border-green-600 space-y-4 w-full'>
          <h2 className="text-xl font-bold text-green-800">Order Summary</h2>
          <div className="flex justify-between text-sm">
            <span>Subtotal</span>
            <span>${total}.00</span>
