    ...
    return (
        <section className="text-center my-16 px-2">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 pt-10">
                {
                    products?.slice(0, 8)?.map(product => <Link to={`/details/${product._id}`} key={product._id} className="group border border-gray-200 rounded-xl shadow-sm hover:shadow-md hover:border-green-400 transition duration-300 p-4 text-center">
